...
`mise run install`, then runs `mise run check:local` for `dagger:check` or
`mise run ci:local` for `dagger:ci`. That keeps task definitions in mise while
Dagger provides the isolated CI execution environment.
