...
datetimes (ASYNC, DTZ). Basedpyright runs in `all` mode and rejects
rule-less or stale `# pyright: ignore` comments. Bandit checks source
security, tests collect
...
heavier analysis profile can use `py:deep` for mypy, documentation coverage
(gating at 100 when run), complexity, dataclass slots, and high-confidence
dead-code checks.
