...
static int table_add(Table *table,
                     const unsigned char *word,
                     size_t length,
                     uint64_t hash)
{
...
static int compare_entries(const void *left_pointer,
                       ...
                       Result *result)
{
    *result = (Result){ 0 };
    ...
    int status = tally_words(&table,
                             data,
                             length,
                             normalize_word_limit(requested_max_word));
    if (status == 0) {
...
static int print_report(const unsigned char *data,
                        size_t length,
    ...
    *options = (Options){
    ...
    for (int i = 1; i < argc; i++) {
        ...
        for (size_t option = 0;
             ...
             option++) {
            const char *value = option_value(number_options[option].name,
                                             argc,
                                             argv,
                                             &i);
            if (value == NULL) {
...
static int read_file(const char *path, unsigned char **data, size_t *length)
{
    ...
    size_t offset = 0;
    while (offset < size) {
        size_t count = fread(buffer + offset, 1, size - offset, file);
        if (count == 0) {
            goto fail;
        }
        offset += count;
...
int main(int argc, char **argv)
{
    ...
    if (read_file(options.path, &data, &length) != 0) {
        fprintf(stderr, "wordcount_c: cannot read %s\n", options.path);
        return 1;
    }
    ...
    if (status != 0) {
        fputs("wordcount_c: out of memory\n", stderr);
        return 1;
    }
    if (fflush(stdout) == EOF || ferror(stdout)) {
        fputs("wordcount_c: cannot write output\n", stderr);
        return 1;
    }
    return 0;
}
